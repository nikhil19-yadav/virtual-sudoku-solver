...
selected_cell = None   # (row, col) of currently selected cell
puzzle_index = 0
h, w = 720, 1280
...
# store number pad rects for detection
num_pad_buttons = {}
...
def draw_ui_elements(image):
    ...
    # Zero button (not needed in Sudoku, but keeping your design)
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Check if number pad clicked
        for num, (x1, y1, x2, y2) in num_pad_buttons.items():
            if x1 <= x <= x2 and y1 <= y <= y2:
                if selected_cell:
                    r, c = selected_cell
                    puzzles[puzzle_index][r][c] = num  # fill number
                break

        # Check if "Next Puzzle" clicked
